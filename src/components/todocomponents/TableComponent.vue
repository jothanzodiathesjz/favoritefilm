    ...
    <div class="w-full  px-4 md:overflow-x-hidden">
      <table role="table" class="w-full min-w-full ">
        ...
        <tbody role="rowgroup" class="px-4">
          <tr role="row" v-for="item in filmData" :key="item.id">
            ...
            <td class="py-3 text-sm " role="cell">
              <div class="flex flex-row font-bold">
                <button v-on:click="btnDetails(item.id)" class="linear rounded bg-lightPrimary p-1 text-base font-medium text-brand-500 transition duration-200 hover:bg-gray-100"><i><ion-icon name="eye-outline"></ion-icon></i></button>
                <button v-on:click="btnUpdate(item.id)" class="linear rounded bg-lightPrimary p-1 text-base font-medium text-brand-500 transition duration-200 hover:bg-gray-100"><i><ion-icon name="pencil-outline"></ion-icon></i></button>
                <button v-on:click="btnDelete(item.id)" class="linear rounded bg-lightPrimary p-1 text-base font-medium text-brand-500 transition duration-200 hover:bg-gray-100"><i><ion-icon name="trash-outline"></ion-icon></i></button>
              </div>
            </td>
          </tr>
